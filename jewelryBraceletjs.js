...
function myFunction() {
  ...
  function dataReportStatus(data) {
    ...
    for (var f = 0; f < (data.length); f++) {
      if (data[f].type.includes("Bracelet")) {
        countf++;
    ...
    for (var i = 0; i < (countf / 4) - 1; i++) {
      var row = document.createElement("div");
      row.setAttribute("class", "row");
      row.style.padding = "10px 0px";
      divall.appendChild(row);

      for (var j = 0; j < countpic + 4; j++) {
        if (data[j].type != "Bracelet") {
          count++;
          countpic++;
        }
        else if (data[j].type == "Bracelet") {
    ...
    for (var h = 0; h < 1; h++) {
      ...
      for (var j = countpic; j < countpic + find; j++) {
        if (data[j].type != "Bracelet") {
          ...
          box.setAttribute("type", "submit")
          if (find == 3) {
            colfinal.style.marginLeft = "280px"
          }
          else if (find == 2) {
